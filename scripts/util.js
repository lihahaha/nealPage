...
const resolveEntry = (entry, cb) => {
    const proto = Object.prototype.toString.call(entry);

    if (/Object/.test(proto)) {
        const newEntry = Object.create(null);

        Object.keys(entry).forEach((e) => {
            newEntry[e] = cb(Array.isArray(entry[e]) ? entry[e] : [entry[e]]);
        });

        return newEntry;
    } else if (/Array/.test(proto)) {
        return entry.map(cb);
    }
...
const getAssetsPublicPath = (config, mime) => process.env.NODE_ENV === 'production' || process.env.NODE_ENV === 'debug'
        ? config.assetsPath[mime] || config.assetsPath.unitive || undefined
        : undefined;
    ...
    compiler.hooks.done.tap('cus_done', (stats) => {
        ...
        if (isSuccessful) {
            ...
            if (isInteractive && isFirstCompile) {
                ...
                firstCompileCallback && firstCompileCallback();
            }
        } else {
...
// 优先级 DRONE_COMMIT_BRANCH >> process.args >> HEAD
function getBranch() {
    let branch = process.env.DRONE_COMMIT_BRANCH;

    if (branch) return branch; // eslint-disable-line

    const argBranch = process.argv.find(arg => arg.indexOf('branch') === 0);

    if (argBranch) {
        branch = argBranch.split('=').pop();
    ...
    try {
        branch = fs
            .readFileSync(path.resolve(__dirname, '../.git/HEAD'), 'utf-8')
            .trim()
            .split('/')
            .pop();
...
function getBranchEnv() {
    ...
    return envMap[branch] ? envMap[branch] : 'live';
}
